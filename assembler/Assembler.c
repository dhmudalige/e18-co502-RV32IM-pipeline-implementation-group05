...
void errorChecker(char *message, char *out_file, int lineNumber, char *fileName);
...
char *OrderChecker(char *type, char *keyword);
...
int main(int argc, char *argv[])
{
    ...
    while (fgets(line, LINE_SIZE, fi) != NULL) // Read a line from the input .s file
    {
        ...
        if (strcmp(type, INVALID) == 0)
            errorChecker("Invalid instruction", out_file, lineNumber, argv[1]);
        count++;
        /**
         * Handling opcodes
        */
        ...
        while (in_token != NULL)
        {
            if (count > operandCountChecker(type, keyword))
                errorChecker("Invalid instruction operand(s)", out_file, lineNumber, argv[1]);
            if (strstr(in_token, "x") && (strstr(in_token, "x") == in_token))
            {
                ...
                {
                    errorChecker("Can't use x0 as a destination register", out_file, lineNumber, argv[1]);
                }
                else if (registerNumber > 31)
                {
                    errorChecker("Invalid register number", out_file, lineNumber, argv[1]);
                }
        ...
        if (strcmp(operandOrder, OrderChecker(type, keyword)) != 0 && !(strcasecmp(keyword, "nop") == 0 || strcasecmp(keyword, "nop\n") == 0))
            errorChecker("Invalid operand order", out_file, lineNumber, argv[1]);
